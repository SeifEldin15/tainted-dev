...
import ShowIPv6Plans from "./showIPv6Plans";
...
const Page = async () => {
  ...
  var userPlans: any = userData?.userIPv6Plans;
  ...
  //       planDataJson?.data?.products?.dataCenterIPV6?.proxy_key;
  //   })
  // );
  ...
  return (
    <div className="h-full w-full">
      {userData?.userPlans?.ipv6 ? (
        <ShowIPv6Plans PlansData={userPlans} />
      ) : (
        <div className="h-full w-full">
          <div className="flex flex-col gap-[10px]">
            ...
            <div className="text-[13px] text-grayText">{`Head to the "IPv6 Proxies" page to buy a plan`}</div>
          </div>
        </div>
      )}
    </div>
