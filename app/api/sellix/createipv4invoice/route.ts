...
import { getCustomIPv4ProxyPrice } from "@/constants/functions";
...
export const POST = async (request: Request) => {
  try {
    ...
    const { name, type, threadCount, days, coupon }: any = await request.json();

    // if the request body is empty then returning the error response
    if (!name || !type || !threadCount || !days) {
    ...
    let planPrice = getCustomIPv4ProxyPrice(Number(threadCount), Number(days));

    // checking if the pricing isnt 0 or less than 0
    if (getCustomIPv4ProxyPrice(Number(threadCount), Number(days)) <= 0) {
      // Returning the error response to the user
      return NextResponse.json<ResponseObjectType>(
        {
          status: "error",
    ...
    const invoice: createSellixInvoiceTypes = {
      title: `${name} | ${threadCount} Threads | ${days} Days`,
      gateway: null,
      gateways: null,
      value: planPrice,
      currency: "USD",
