...
import {IPv4PricingInfo, IPv6PricingInfo} from "@/constants/functions";
          ...
          >
            ...
            <div className="mt-3 flex flex-col-reverse justify-between gap-5 sm:flex-row sm:items-end">
              ...
              <div className="grid grid-flow-row sm:grid-cols-2 md:grid-cols-3 w-full gap-3">
                ...
                <div className="flex flex-row items-center gap-2.5">
                  <div className="h-2.5 w-2.5 rounded-full bg-brand"></div>
                  <div className="text-white/80">User/Password or IP Auth</div>
                </div>
                <div className="flex flex-row items-center gap-2.5">
                  <div className="h-2.5 w-2.5 rounded-full bg-brand"></div>
                ...
                <div className="flex flex-row items-center gap-2.5">
                  <div className="h-2.5 w-2.5 rounded-full bg-brand"></div>
                  <div className="text-white/80">Sticky Session Support</div>
                </div>
              </div>
            </div>
            {/* plans */}
            <div className="mt-3 grid sm:grid-cols-2 md:grid-cols-3 grid-flow-row gap-6">
              {IPv4PricingInfo?.map((plan, index) => (
                ...
                >
                  <div className="text-3xl text-brand">{`${plan?.threads} Threads`}</div>
                  <div className="flex flex-col gap-3">
                    <Button
                        variant={"outline"}
                        onClick={() => redirectTo("https://eclipseproxy.com/dashboard")}
                    >{`1 Day - ${plan?.prices?.day}$`}</Button>
                    <Button
                      variant={"outline"}
                      onClick={() => redirectTo("https://eclipseproxy.com/dashboard")}
