      ...
      <div className="bg-white dark:bg-gray-800 p-4 sm:p-8 border border-gray-200 dark:border-transparent rounded-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {plans.map((plan, index) => (
            <div key={index} className="border border-gray-200 dark:border-transparent rounded-lg p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl text-brand font-semibold mb-3 sm:mb-4">{plan.threads}</h3>
              {plan.pricing.map((price, priceIndex) => (
                <button
                  key={priceIndex}
                  className="w-full py-2 sm:py-3 px-3 sm:px-4 mb-2 sm:mb-3 text-sm sm:text-base border border-gray-200 dark:border-gray-700 rounded-lg hover:border-brand transition-colors text-gray-900 dark:text-white"
                >
