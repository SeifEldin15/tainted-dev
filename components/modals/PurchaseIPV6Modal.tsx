...
export function PurchaseIPV6Modal() {
  ...
  // console.log("[MODAL] PurchaseIPV6Modal", currentModal.modalData);
  ...
  const handleCreateInvoice = async () => {
    ...
    let promise = new Promise(async (resolve, reject) => {
      ...
      const response = await createSellixIPV6Invoice({
        name: "IPv6 Proxies",
        type: "IPV6",
        days: modalData?.days,
  ...
  async function checkCoupon() {
    const promise = new Promise(async (resolve, reject) => {
      const response = await getCoupon(coupon, "ipv6_dc");
      if (response.status === "success") {
  ...
  return (
    <Dialog open={currentModal.modalName === "PurchaseIPV6Modal"}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{`Purchase IPv6 Proxy`}</DialogTitle>
        ...
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex flex-col w-full py-2 text-[#c7c7c7] gap-1.5">
            ...
            <div className="flex flex-row items-center justify-between">
              <div>{`Plan Type`}</div>
              <div className="text-brand font-semibold">{`IPv6 Proxies`}</div>
            </div>
            <div className="flex flex-row items-center justify-between">
