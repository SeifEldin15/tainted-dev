...
export function PurchaseCoreResiModal() {
  ...
  const handleCreateInvoice = async () => {
    ...
    let promise = new Promise(async (resolve, reject) => {
      ...
      const response = await createSellixCoreResiInvoice({
        name: "Core Residential Proxies",
  ...
  async function checkCoupon() {
    const promise = new Promise(async (resolve, reject) => {
      const response = await getCoupon(coupon, "core_resi");
      if (response.status === "success") {
  ...
  return (
    <Dialog open={currentModal.modalName === "PurchaseCoreResiModal"}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{`Purchase Core Residential Proxy`}</DialogTitle>
        ...
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex flex-col w-full py-2 text-[#c7c7c7] gap-1.5">
            ...
            <div className="flex flex-row items-center justify-between">
              <div>{`Plan Type`}</div>
              <div className="text-brand font-semibold">{`Core Residential Proxies`}</div>
            </div>
            <div className="flex flex-row items-center justify-between">
