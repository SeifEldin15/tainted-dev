...
import { createSellixIPV4Invoice, getCoupon } from "@/constants/functions";
import toast from "react-hot-toast";
import { PROMISE_MESSAGES } from "@/constants/Messages";
import { useRouter } from "next/navigation";

export function PurchaseIPV4Modal() {
  ...
  // console.log("[MODAL] PurchaseIPV4Modal", currentModal.modalData);
  ...
  const modalData: any = currentModal?.modalData;
  ...
  const handleCreateInvoice = async () => {
    ...
    let promise = new Promise(async (resolve, reject) => {
      ...
      const response = await createSellixIPV4Invoice({
        name: "IPv4 Proxies",
        type: "IPV4",
        days: modalData?.days,
        threadCount: modalData?.threadCount,
  ...
  async function checkCoupon() {
    const promise = new Promise(async (resolve, reject) => {
      const response = await getCoupon(coupon, "ipv4_dc");
      if (response.status === "success") {
  ...
  const couponDiscount =
    (appliedCoupon?.percent / 100) * Number(modalData?.price) || 0;
  return (
    <Dialog open={currentModal.modalName === "PurchaseIPV4Modal"}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{`Purchase IPv4 Proxy`}</DialogTitle>
          <DialogDescription className="text-xs">{`Plan will be automatically added after the payment has confirmed.`}</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex flex-col w-full py-2 text-[#c7c7c7] gap-1.5">
            ...
            <div className="flex flex-row items-center justify-between">
              <div>{`Plan Type`}</div>
              <div className="text-brand font-semibold">{`IPv4 Proxies`}</div>
            </div>
            <div className="flex flex-row items-center justify-between">
              <div>{`Amount (in USD)`}</div>
              <div
                className={`${appliedCoupon.percent > 0 ? "line-through" : ""}`}
              >{`$${Number(modalData?.price)}`}</div>
            </div>
            {appliedCoupon?.percent > 0 && (
              <>
                <div className="flex flex-row items-center justify-between">
                  ...
                  <div>
                    {`${appliedCoupon?.percent}% x ${Number(modalData?.price)}`}{" "}
                    = ${couponDiscount.toFixed(1) || "0.0"}
                  </div>
                </div>
                <div className="flex flex-row items-center justify-between">
                  <div>{`Final Amount`}</div>
                  <div>{`$${(Number(modalData?.price) - couponDiscount).toFixed(
                    1
                  )}`}</div>
                </div>
              </>
            ...
            <div className="flex flex-row items-center justify-between">
              <div>{`Days`}</div>
              <div>{`${modalData?.days}`}</div>
            </div>
            <div className="flex flex-row items-center justify-between">
              <div>{`Threads`}</div>
              <div>{`${modalData?.threadCount}`}</div>
            </div>
          </div>
          {/* Devider */}
